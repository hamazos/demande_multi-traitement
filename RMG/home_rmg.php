                ...
                <div class="card-body">
                    <div class="table-responsive"style="margin-top: 20px;">
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody> 
                            <?php
                                    $sql = "SELECT * FROM demande WHERE (verif_RMG=1 AND verif_Info=1) OR (verif_RMG=0 AND verif_Info=1) OR (verif_RMG=2 AND verif_Info=2) OR (verif_RMG=3 AND verif_Info=3)";
                                    $query = $conn->query($sql);
                                    while($row = $query->fetch_assoc()){
                                        if($row['verif_Info']==1 && $row['statut']==0){
                                            $statut='<button type="button" class="btn btn-primary disabled"><i class="fas fa-sync-alt fa-spin"></i> En cours</button>';
                                        }if($row['verif_RMG']==1){
                                            $statut='<button type="button" class="btn btn-info disabled"><i class="fas fa-sync-alt fa-spin"></i> Accepter préliminaire</button>';
                                        }elseif($row['verif_RMG']==2){
                                            $statut='<button type="button" class="btn btn-success disabled"><i class="fas fa-check"></i>Accepter  </button>';
                                        }elseif($row['verif_RMG']==3){
